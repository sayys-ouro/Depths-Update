...
/**
 * Classic improved Perlin noise at a single scale. Faithful backport of modern Minecraft's ImprovedNoise.
 * Uses 256-entry permutation table and 16 gradient vectors with smootherstep interpolation.
 */
public final class ImprovedNoise {
    ...
    public double noise(double _x, double _y, double _z, double yScale, double yFudge) {
        ...
        double y = _y + this.yo;
        double z = _z + this.zo;
        int xf = NoiseUtils.floor(x);
        int yf = NoiseUtils.floor(y);
        int zf = NoiseUtils.floor(z);
        ...
        if (yScale != 0.0) {
            ...
            yrFudge = (double) NoiseUtils.floor(fudgeLimit / yScale + (double) SHIFT_UP_EPSILON) * yScale;
        } else {
            yrFudge = 0.0;
        }
    ...
    private double sampleAndLerp(int x, int y, int z, double xr, double yr, double zr, double yrOriginal) {
        ...
        double xAlpha = NoiseUtils.smoothstep(xr);
        double yAlpha = NoiseUtils.smoothstep(yrOriginal);
        double zAlpha = NoiseUtils.smoothstep(zr);
        return NoiseUtils.lerp3(xAlpha, yAlpha, zAlpha, d000, d100, d010, d110, d001, d101, d011, d111);
    }
    ...
    private double sampleWithDerivative(int x, int y, int z, double xr, double yr, double zr, double[] derivativeOut) {
        ...
        double xAlpha = NoiseUtils.smoothstep(xr);
        double yAlpha = NoiseUtils.smoothstep(yr);
        double zAlpha = NoiseUtils.smoothstep(zr);
        double d1x = NoiseUtils.lerp3(xAlpha, yAlpha, zAlpha,
                (double) g000[0], (double) g100[0], (double) g010[0], (double) g110[0],
                (double) g001[0], (double) g101[0], (double) g011[0], (double) g111[0]);
        double d1y = NoiseUtils.lerp3(xAlpha, yAlpha, zAlpha,
                (double) g000[1], (double) g100[1], (double) g010[1], (double) g110[1],
                (double) g001[1], (double) g101[1], (double) g011[1], (double) g111[1]);
        double d1z = NoiseUtils.lerp3(xAlpha, yAlpha, zAlpha,
                (double) g000[2], (double) g100[2], (double) g010[2], (double) g110[2],
                (double) g001[2], (double) g101[2], (double) g011[2], (double) g111[2]);
        double d2x = NoiseUtils.lerp2(yAlpha, zAlpha, d100 - d000, d110 - d010, d101 - d001, d111 - d011);
        double d2y = NoiseUtils.lerp2(zAlpha, xAlpha, d010 - d000, d011 - d001, d110 - d100, d111 - d101);
        double d2z = NoiseUtils.lerp2(xAlpha, yAlpha, d001 - d000, d101 - d100, d011 - d010, d111 - d110);
        double xSD = NoiseUtils.smoothstepDerivative(xr);
        double ySD = NoiseUtils.smoothstepDerivative(yr);
        double zSD = NoiseUtils.smoothstepDerivative(zr);
