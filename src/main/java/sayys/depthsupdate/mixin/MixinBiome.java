...
public abstract class MixinBiome {
    ...
    /**
     * @author __sayys
     * @reason Expand bedrock downward to -64.
     */
    @Overwrite
    public final void generateBiomeTerrain(@NonNull World worldIn, @NonNull Random rand, ChunkPrimer chunkPrimerIn, int x, int z,
                                           double noiseVal) {
        ...
        for (int j1 = 255; j1 >= -64; --j1) { // Changed bounds from >= 0 to >= -64
            // Changed bedrock threshold from `j1 <= rand.nextInt(5)` to `j1 <= -64 +
            // rand.nextInt(5)`
            if (j1 <= -64 + rand.nextInt(5)) {
